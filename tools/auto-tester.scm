...
|#				  
		    ...
		    "(*s7* 'rootlet-size)"
		    ;"(*s7* 'stack)" "(*s7* 'stack-size)" "(*s7* 'stack-top)" ; variable
		    "(*s7* 'version)"
...
#|
	  (unless (or (string-position "error" str)
		      (string-position "throw" str))
	    ;(format *stderr* "info: ~S~%   from ~S~%" info str)
	    (when (pair? info) (apply format #f info)))
|#
