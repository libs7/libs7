...
const char *sexpfile_to_string(const char *sexpfile_name)
{
...
s7_pointer fix_sexpfile(s7_scheme *s7, const char *sexpfile_name)
{
    ...
    const char *sexpstring = sexpfile_to_string(sexpfile_name);
...
/*
 * called by _g_read_sexp using s7_call_with_catch
 * precondition: g_sexp_inport is set in local env (curlet)
 */
...
/* s7_pointer _sexp_read_port(s7_scheme *s7, s7_pointer inport) */
{
    ...
    s7_pointer _sexps = s7_let_ref(s7, s7_curlet(s7),
                                   s7_make_symbol(s7, "-sexp-sexps"));
...
#if defined(DEVBUILD)
    log_debug("reading stanzas from sexpfile: %s", sexpfile);
#endif
...
static s7_pointer g_sexp_read(s7_scheme *s7, s7_pointer args)
{
    ...
    // called this routine (i.e. the c stack), NOT the assignment to
    // result below.
    s7_pointer result = s7_eval_c_string_with_environment(s7, "(apply -g-sexp-read xargs)", env);
...
s7_pointer libsexp_s7_init(s7_scheme *s7)
{
  ...
  _sexp_read_catcher_s7 =
      s7_make_function(s7, "-sexp-read-thunk-catcher",
                         _sexp_read_catcher,
