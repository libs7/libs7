...
static int iwrap_put(void *closure, const char *name, int escape, FILE *file)
{
	...
	sbuf_reset(&sbuf);
#ifdef DEBUGGING
        log_debug("iwrap_put calling iwrap->get");
#endif
	rc = iwrap->get(iwrap->closure, name, &sbuf);
#ifdef DEBUGGING
        log_debug("iwrap->get rc: %d", rc);
        log_debug("iwrap->get sbuf.value: %s", sbuf.value);
        log_debug("iwrap->get sbuf.lambda: %d", sbuf.lambda);
#endif
        if (sbuf.value == NULL)
...
static int process(const char *template, size_t template_length, struct iwrap *iwrap, FILE *file, struct prefix_s *prefix, int escape)
{
	...
	for (;;) {
            ...
            /* dump_closure(((struct Xwrap*)iwrap->closure)->closure); */
            if (iwrap->dump_closure) {
                log_debug("dump_closure");
                iwrap->dump_closure(iwrap->closure);
            }
...
/* #endif */
            ...
            case '>': /* partials */
                ...
                if (enabled) {
                    sbuf_reset(&sbuf);
                    rc = iwrap->partial(iwrap->closure_partial, key, &sbuf);
                    if (rc >= 0) {
            ...
            default:
                ...
                if (enabled) { // iwrap_put inlined
                    ...
                    sbuf_reset(&sbuf);
#ifdef DEBUGGING
                    log_debug("calling iwrapper->get");
                    /* log_debug("sbuf.releasecb: %x", sbuf.releasecb); */
#endif
                    rc = iwrapper->get(iwrapper->closure, key, &sbuf);
#ifdef DEBUGGING
                    log_debug("iwrapper->get rc: %d", rc);
                    log_debug("iwrapper->get sbuf.value: %s", sbuf.value);
                    log_debug("iwrapper->get sbuf.lambda: %d", sbuf.lambda);
                    log_debug("sbuf.releasecb: %x", sbuf.releasecb);
#endif
                    if (rc >= 0) {
                        length = sbuf_length(&sbuf);
...
int mustach_file(const char *template, size_t length, const struct mustach_itf *itf, void *closure, int flags, FILE *file)
{
	...
	iwrap.get = itf->get;
	iwrap.flags = flags;
	iwrap.dump_closure = itf->dump_closure;
