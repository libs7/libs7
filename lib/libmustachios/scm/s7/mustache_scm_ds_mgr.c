...
static char *getkey(char **head, int sflags)
{
    ...
    if (!car)
        ...
        //NB Json Pointers not supported in mustache_s7
...
//(struct wrap *w, const char *name)
{
    ...
    /* case of . alone if Mustach_With_SingleDot? */
        log_debug("CASE '.'");
#endif
        if (copy[1] == '\0') { // singledot always enabled
...
#ifdef DEVBUILD
            log_debug("DOTTEDNAME, copy[1]: '%c'", copy[1]);
#endif
            // op is '.foo' - ??
            result = S_none;
        }
